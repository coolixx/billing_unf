...
Функция ПолучитьНастройкуБиллингаПоID(Идентификатор) Экспорт
	
	НастройкаБиллинга = Неопределено;
	
	Отбор = Новый Структура;
	Отбор.Вставить("Идентификатор", Идентификатор);
	
	Выборка = РегистрыСведений.BS_КонтрагнетыОблачногоСервиса.Выбрать(Отбор);
	
	Если Выборка.Следующий() Тогда
		НастройкаБиллинга = Выборка.НастройкаБиллинга;
	КонецЕсли;
	
	Возврат НастройкаБиллинга;
